...
export function fastICA(signals, options) {
  ...
  //   throw new Error(
  //     "Negentropy approximation function must be either 'exp' or 'logcosh'"
  //   );
  // }

  let eigs = window.numeric.eig(stat.cov(transpose(signals)));
  let V = eigs.E.x;
  let D = eigs.lambda.x;
  ...
  let whitened = multiply(whitening, centered);
